...
import { Linkedin, Mail, KeyIcon, LogOut, ArrowLeft } from "lucide-react";
...
const SignInPage = () => {
  ...
  const [mode, setMode] = useState<"signIn" | "requestReset" | "enterCode">("signIn");
  ...
  const handleEmailPasswordSignIn = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      if (err.errors?.[0]?.code === "session_exists") {
        ...
        setError("You're currently signed in to another account. Please sign out first.");
      } else {
        setError("An error occurred during sign-in. Please check your credentials and try again.");
      }
    }
  };
      ...
      <Card className="w-full max-w-md border-none shadow-lg overflow-hidden relative z-10">
        <div className="px-6 pt-8 pb-6 bg-gradient-to-r from-indigo-600 to-purple-600 rounded-t-lg">
          <h2 className="text-3xl font-extrabold text-white text-center">
            {mode === "signIn" ? "Welcome Back" : mode === "requestReset" ? "Reset Password" : "Enter Reset Code"}
          </h2>
        </div>
        <div className="p-6 bg-white rounded-b-lg">
          {mode === "signIn" && (
            <>
              <form onSubmit={handleEmailPasswordSignIn} className="space-y-4">
                ...
                {existingSession ? (
                  <Button onClick={handleSignOut} className="w-full bg-red-500 text-white py-2 rounded-md hover:bg-red-600 transition-all duration-300">
                    <LogOut className="w-4 h-4 mr-2" />
                    Sign Out of Existing Session
                  </Button>
                ) : (
                  <Button type="submit" className="w-full bg-gradient-to-r from-indigo-600 to-purple-600 text-white py-2 rounded-md hover:from-indigo-700 hover:to-purple-700 transition-all duration-300">
          ...
          {mode === "requestReset" && (
            <form onSubmit={handleRequestPasswordReset} className="space-y-4">
              ...
              <Button type="button" variant="link" onClick={() => setMode("signIn")} className="w-full mt-2 text-indigo-600 hover:text-indigo-500">
