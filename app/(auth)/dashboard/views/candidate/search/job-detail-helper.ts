...
import { getJobNode, getAllJobRelationships, JobNode, NodeWithId } from "@/lib/jobs/mutation";
import { addCandidateJobBookmark, removeCandidateJobBookmark, checkCandidateJobBookmarkExists } from './bookmark';
...
export async function fetchJobDetails(jobId: string) {
    try {
      ...
      const bookmarkExists = await checkCandidateJobBookmarkExists(jobId, 'candidateId');
  
      // Remap job properties
      const remappedProperties = await remapJobProperties(details.job_type, details.location_type, details.security_clearance);
...
export async function toggleBookmark(jobId: string, isCurrentlyBookmarked: boolean) {
  try {
