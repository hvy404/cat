...
const JobMoreDetails: React.FC<JobDetailsProps> = ({ jobId, onBack }) => {
  ...
  const handleBookmarkToggle = async () => {
    try {
      const newBookmarkState = await toggleBookmark(jobId, isBookmarked);
      setIsBookmarked(newBookmarkState);
    } catch (err) {
      console.error("Error toggling bookmark:", err);
      setError("Failed to toggle bookmark. Please try again later.");
      setIsErrorDialogOpen(true);
    }
  };
      ...
      <Card>
        ...
        <CardContent className="space-y-2">
          <div className="flex items-center">
            ...
            <span className="font-semibold text-sm">{jobDetails.company}</span>
          </div>
          <div className="flex items-center">
          ...
          <p className="text-sm text-gray-600">{jobDetails.company_overview}</p>
        </CardContent>
      </Card>
    </div>
    </div>
